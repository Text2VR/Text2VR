...
# -*- coding: utf-8 -*-
"""
Indoor-first panorama segmentation for VR assets (panorama-aware).

Key additions vs. baseline:
- Wrap-aware IoU/NMS so objects split across the 360° seam are not duplicated.
- Mask wrap-merge & cleanup so seam-split masks become a single clean mask.
- Optional "floor contact" filter to keep only floor-anchored interactive items.
- Window/door exclusion uses mask-first policy with bottom-center "indoor anchor".
...
# ==============================
MIN_PROMPTS = 1
MAX_PROMPTS = 5                 # allow more labels to compete
BOX_THRESHOLD = 0.20            # tuned lower to catch near-window items
TEXT_THRESHOLD = 0.15           # tuned lower to catch near-window items
MIN_AREA_RATIO = 0.005          # tuned lower to not miss small assets
MAX_AREA_RATIO = 0.30

# Preferred order to ensure at least one object remains
PREFERRED_ORDER = [
...
EXCLUSION_CENTER_RULE = True    # conservative rule if mask is unavailable

# Mask-based indoor/outdoor split (preferred)
EXCLUSION_USE_MASK = True
EXCLUSION_MASK_DILATE_PX = 4    # tuned smaller per request
EXCLUSION_OVERLAP_DROP = 0.80   # tuned stricter per request

# Panorama-aware cross-label NMS
WRAP_NMS_IOU = 0.45             # seam-aware IoU threshold

# Floor-anchored categories (considered "interactive")
FLOOR_ANCHORED = {
    "sofa","couch","armchair","chair","bench","stool",
    "table","coffee table","side table",
    "plant","cabinet","bookshelf","tv","television","floor lamp"
}
...
def iou_xyxy(a: List[float], b: List[float]) -> float:
    """Standard IoU for [x1,y1,x2,y2] boxes (no wrap)."""
    ax1, ay1, ax2, ay2 = a
    bx1, by1, bx2, by2 = b
...
def iou_xyxy_wrap(a: List[float], b: List[float], W: int) -> float:
    """Wrap-aware IoU for equirectangular panoramas: try shifting box b by ±W."""
    best = 0.0
    for shift in (-W, 0, W):
        b2 = [b[0] + shift, b[1], b[2] + shift, b[3]]
        best = max(best, iou_xyxy(a, b2))
    return best
...
# ==============================
# Panorama wrap mask utils
# ==============================
def wrap_merge_and_clean(mask: np.ndarray, min_region_ratio: float = 0.001, close_ks: int = 7) -> np.ndarray:
    """Merge seam-split pieces by tiling 3x width, keep center-connected components, remove tiny islands, then smooth."""
    H, W = mask.shape
    # 3x wide canvas to connect components across left/right seam
    ...
    num, lbl, stats, _ = cv2.connectedComponentsWithStats(wide, connectivity=8)
    merged = np.zeros_like(wide)
    center_lbl = lbl[:, W:2 * W]  # labels overlapping the center tile
    center_ids = np.unique(center_lbl)
    for i in center_ids:
        if i == 0:
            continue
        merged[lbl == i] = 1

    merged = merged[:, W:2 * W]  # back to original width

    # Remove tiny components
    min_area = int(min_region_ratio * H * W)
    num2, lbl2, stats2, _ = cv2.connectedComponentsWithStats(merged, connectivity=8)
    cleaned = np.zeros_like(merged)
    for i in range(1, num2):
        if stats2[i, cv2.CC_STAT_AREA] >= min_area:
            cleaned[lbl2 == i] = 1

    # Smooth (close small gaps)
    if close_ks > 0:
...
# ==============================
# Floor contact filter
# ==============================
def touches_floor(mask: np.ndarray, floor_band_ratio: float = 0.12) -> bool:
    """Return True if any positive pixel touches the bottom floor band (approx. floor contact)."""
    H, W = mask.shape
    band_top = int(H * (1.0 - floor_band_ratio))
    band_top = np.clip(band_top, 0, H - 1)
    return bool(np.any(mask[band_top:, :] > 0))
...
# ==============================
def main(args):
    print("🚀 Starting Panorama Segmentation Pipeline (panorama-aware)")
    print("=" * 60)
    ...
    # Detection → wrap-aware cross-label NMS → SAM segmentation
    ...
    for prompt in final_labels:
        ...
        for d in dets:
            ...
            # Wrap-aware cross-label NMS
            if any(iou_xyxy_wrap(box, kb, W) >= args.wrap_nms_iou for kb in kept_boxes):
                continue
        ...
        # Optional floor-contact filter to drop wall/ceiling-attached items
        if args.enable_floor_filter and len(masks) > 0:
            filtered_good, filtered_masks = [], []
            for d, m in zip(good, masks):
                # decide by target label (prefer prompt for stability)
                tgt_label = prompt.lower()
                if (tgt_label in FLOOR_ANCHORED) and (not touches_floor(m, floor_band_ratio=args.floor_band_ratio)):
                    continue
                filtered_good.append(d); filtered_masks.append(m)
            good, masks = filtered_good, filtered_masks
    ...
    for prompt, dets in all_results.items():
        ...
        # Panorama wrap merge + cleanup before saving
        combined_mask = wrap_merge_and_clean(
            (combined_mask > 0).astype(np.uint8),
            min_region_ratio=args.min_region_ratio,
            close_ks=args.close_kernel
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Indoor-first panorama segmentation (success-only, panorama-aware).")
    ...
    # Thresholds & ratios (reflect tuned defaults)
    ...
    # Panorama-aware NMS
    parser.add_argument("--wrap_nms_iou", type=float, default=WRAP_NMS_IOU,
                        help="IoU threshold for wrap-aware cross-label NMS across the 360° seam.")

    # Floor-contact filtering
    parser.add_argument("--enable_floor_filter", type=lambda x: str(x).lower() not in ["0","false","no"],
                        default=False, help="Keep only floor-anchored interactive items (drop wall/ceiling fixtures).")
    parser.add_argument("--floor_band_ratio", type=float, default=0.40,
                        help="Bottom band ratio used to test 'touches floor'. Larger → stricter.")
    # Wrap mask merge & cleanup parameters
    parser.add_argument("--min_region_ratio", type=float, default=0.001,
                        help="Minimum connected-component area (ratio of image) to keep in final mask.")
    parser.add_argument("--close_kernel", type=int, default=7,
                        help="Morphological close kernel size for mask smoothing.")
...
BOX_THRESHOLD = 0.20
TEXT_THRESHOLD = 0.15
MIN_AREA_RATIO = 0.005          # keep boxes covering at least 1% of the image
...
# Mask-based indoor/outdoor split (preferred)
EXCLUSION_USE_MASK = True
EXCLUSION_MASK_DILATE_PX = 4      # dilation (px) applied to union of window/door masks
EXCLUSION_OVERLAP_DROP = 0.80      # drop if (exclusion-mask coverage within candidate box) ≥ this
