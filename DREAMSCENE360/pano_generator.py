...
def generate_panorama(text_prompt,
                      output_dir="panorama_output",
                      api_key=None,
                      use_self_refinement=False,
                      num_prompt=3,
                      max_rounds=3):
    """Generate a panorama image from a text prompt (optionally with self-refinement)."""
    print(f"🎨 Generating panorama from: '{text_prompt}'")

    # Ensure output directory exists
    os.makedirs(output_dir, exist_ok=True)

    # Use scene folder name as a stable foldername for self-refinement
    # e.g., output_dir=/workspace/DREAMSCENE360/data/my_scene  -> foldername="my_scene"
    scene_foldername = os.path.basename(os.path.normpath(output_dir)) or "panorama_scene"
    ...
    if use_self_refinement and api_key:
        ...
        runner = Text2PanoRunner(
            api_key=api_key,
            testfile=text_file,
            num_prompt=num_prompt,        # pass-through
            max_rounds=max_rounds,        # pass-through
            foldername=scene_foldername   # stable, short folder name
        )
        runner.run_command(filename='text2pano_self_refine_pipeline.py')

        # Expected final best path produced by the pipeline
        best_rel = os.path.join("self_refinement", scene_foldername, "iter_best", "image.png")
        if os.path.exists(best_rel):
            final_path = os.path.join(output_dir, "panorama.png")
            try:
                shutil.copy2(best_rel, final_path)
                return final_path
...
def main():
    ...
    # NEW: make rounds controllable from CLI
    parser.add_argument('--num_prompt', type=int, default=3, help='Candidates per round for self-refinement')
    parser.add_argument('--max_rounds', type=int, default=3, help='Number of refinement rounds')
    ...
    pano_path = generate_panorama(
        ...
        num_prompt=args.num_prompt,   # pass-through
        max_rounds=args.max_rounds    # pass-through
    )
